...
class User(models.Model):
    ...
    role = models.CharField(choices=[(choice.value, choice.name) for choice in Role])
...
class Anime(models.Model):
    ...
    genre = models.CharField(choices=[(choice.value, choice.name) for choice in Genre])
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            ...
            max_id = Anime.objects.all().aggregate(Max('id'))['id__max'] or 0
            self.id = max_id + 1
...
class MediaFile(models.Model):
    ...
    video = models.FileField(upload_to='yummy/%y', max_length=250, null=True, validators=[
        FileExtensionValidator(allowed_extensions=['MOV', 'avi', 'mp4', 'webm', 'mkv'])])


class Folder(models.Model):
    name = models.CharField(max_length=30, null=False, blank=False)
    colour = models.CharField(choices=[(choice.value, choice.name) for choice in Colour])
    id_anime = models.ManyToManyField(Anime)
    id_subscriber = models.ForeignKey(Subscriber, on_delete=models.CASCADE)
    favourites = models.ManyToManyField(Anime, related_name='favorited_by')
